  ...
  <div class="profile_detail_container">
    ...
    <div class="profile_detail_items">
      <div class="profile_detail_item"
           data-path-name="DetailHeadIcon"
           :data-image="assets.maleAvatar"
           @click="navTo">
        <div class="profile_detail_item_label pen"
             :style="{width: calcToRealPx(300) + 'px', height: calcToRealPx(150) + 'px'}">
          头像
        </div>
        <div class="profile_detail_item_value pen"
             :style="{width: calcToRealPx(300) + 'px', height: calcToRealPx(150) + 'px'}">
      ...
      <div class="main_operation_item"
           ...
           :data-name="op.name"
           :data-title="op.label"
           :data-component="op.component"
           @click="detailItemClick">
        <div class="main_operation_item_inner pen">
          <div class="main_operation_item_label"
               v-text="op.label"
               :style="{width: calcToRealPx(200) + 'px'}"></div>
          <div class="main_operation_item_right"
               :style="{width: calcToRealPx(400) + 'px'}">
            <div class="main_operation_item_right_value"
                 v-if="op.name.toLowerCase() === 'phonenum'">{{personInfo[op.name.toLowerCase()] | phonenum}}</div>
            <div class="main_operation_item_right_value"
                 v-else-if="op.name.toLowerCase() === 'gender'">{{personInfo[op.name] | getGenderName}}</div>
            <div class="main_operation_item_right_value"
                 v-else
                 v-text="personInfo[op.name.toLowerCase()]"></div>
            <x-icon type="ios-arrow-right"
                    size="16"
                    class="icon_color_normal"
    ...
    <div class="btns_container"
         :class="{shown: hasChanged}">
      <x-button type="primary"
                text="修改"
                :disabled="!hasChanged"
                @click.native="updateUserInfo"></x-button>
    </div>
  </div>
</template>
<style scoped>
